...
int32_t wifi_manager_get_rssi(void) {
    ...
    if (esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK) {
        ...
        ESP_LOGE(TAG_WIFI, "Failed to get AP info");
...
void wifi_print_stats(void *pvParameters) {
    ...
    while (1) {
        ...
        if (is_connected) {
            ESP_LOGI(TAG_WIFI, "~~~~~~~~~~~");
            ESP_LOGI(TAG_WIFI, "IP:" IPSTR, IP2STR(&ip.ip));
            ESP_LOGI(TAG_WIFI, "MASK:" IPSTR, IP2STR(&ip.netmask));
            ESP_LOGI(TAG_WIFI, "GW:" IPSTR, IP2STR(&ip.gw));
            ESP_LOGI(TAG_WIFI, "RSSI: %ld", (long int)ap_info.rssi);
            ESP_LOGI(TAG_WIFI, "~~~~~~~~~~~");
...
esp_err_t wifi_set_config(const char* ssid, const char* pass) {
    ...
    ESP_LOGI(TAG_WIFI, "Set WiFi configuration SSID %s...", wifi_config.sta.ssid);
